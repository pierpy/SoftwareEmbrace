function [comparison,means,h,gnames,t,crit] = my_multcompare(stats,varargin)
...
% Reshapes the covariance matrix.
...
% Calculates the confidence interval for each comparison.
delta = crit * se;
...
if dodisp
    ...
    d = zeros ( ngroups, ngroups );
    d(i12) = se;
    sum1 = sum(sum(d));
    d = d + d';
    sum2 = sum(d);
    if (ng > 2)
        w = ((ngroups-1) * sum2 - sum1) ./ ((ngroups-1)*(ngroups-2));
    else
        w = sum1 * ones(2, 1) / 2;
    end
    halfwidth = crit * w(:);
