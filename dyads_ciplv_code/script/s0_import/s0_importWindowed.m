...
config.path.sens = '../../data/segments_window/';
config.path.patt = '*_*';

% Sets the windowing options.
config.len       = 149;
config.win.padd  = 2;
config.win.len   = 4;
config.win.step  = 2;
...
for dindex = 1: numel ( dyads )
    ...
    % Gets the number of windows per file.
    ulen  = config.len - 2 * config.win.padd;
    nwin  = ( ulen - ( config.win.len - config.win.step ) ) / config.win.step;
    nwin  = floor ( nwin );

    % Iitializes the trial data structure.
    trialdata           = [];
    trialdata.label     = {};
    trialdata.time      = cell ( nfile * nwin, 1 );
    trialdata.trial     = cell ( nfile * nwin, 1 );
    trialdata.fsample   = [];
    trialdata.trialinfo = nan ( nfile * nwin, 3 );
    ...
    for findex = 1: numel ( files )
        ...
        % Initializes the datas and times cell arrays.
        times  = cell ( nwin, 1 );
        datas  = cell ( nwin, 1 );
        ...
        for windex = 1: nwin
            ...
            offset = ( windex - 1 ) * config.win.step * srate;
            mask   = offset + ( 1: config.win.len * srate + 2 * config.win.padd * srate );

            % Stores the current window.
            times { windex } = time ( mask );
            datas { windex } = data ( :, mask );
        end


        % Calculates the offset and the mask for this file.
        offset = ( findex - 1 ) * nwin;
        mask   = offset + ( 1: nwin );
        
        % Stores the data and time.
        trialdata.time  ( mask ) = times;
        trialdata.trial ( mask ) = datas;

        % Stores the condition.
        trialdata.trialinfo ( mask, 1 ) = findex;
        trialdata.trialinfo ( mask, 2 ) = cond1;
        trialdata.trialinfo ( mask, 3 ) = cond2;
    ...
    trialinfo.trialpad  = 2 * ones ( nfile * nwin, 1 );
