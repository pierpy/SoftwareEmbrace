...
for findex = 1: numel ( files )
    ...
    for dindex = dindexes'
        ...
        cfg.channel         = cat ( 2, sketchdata.compinfo.SOBI.topolabel (:)', { 'EOG' 'ECG' 'EMG' } );
    ...
    wholedata           = myft_appendtrials ( cfg, wholedatas {:} );
    clear wholedatas
    ...
    if config.deEOG.perform && any ( comptype == config.deEOG.comptype )
        ...
        if any ( config.deEOG.filter )
            ...
            fir     = fir1 ( 4 * 200, config.deEOG.filter / ( trialdata.fsample / 2 ), 'high' );
