function [ p, table, stats, terms ] = my_anovan_lite ( data, group, varargin )
...
switch sstype
    case 1
        ...
        findex = nterms;                % locations of model with all terms
    case 2
        modw = termsnotcontained ( fulltermlist, continuous, dohier );
        findex = ( sum ( modw, 2 ) == nterms );
        TnotC = modw;
    case 3
        modw = true ( nterms );
        findex = 1: nterms;
...
%     table ( end, 4, : ) = num2cell (0);
    table { end, 2, : } = sst;
    table { end, 3, : } = repmat ( ncases - 1, 1, numel ( sst ) );
    table { end, 4, : } = zeros ( 1, numel ( sst ) );
...
if all ( dfTerm (:) > 0 )
   ...
   for j = find ( any ( varest > 0, 2 )' )
      ...
      cU = zeros ( 1, ncomps );
      if any ( t )
         cL = cL + sum ( Rcoeffs ( t ) .*  L ( Rterms ( t ), : ), 1 );
         cU = cU + sum ( Rcoeffs ( t ) .*  U ( Rterms ( t ), : ), 1 );
      end
      if any ( ~t )
         cL = cL + sum ( Rcoeffs ( ~t ) .*  U ( Rterms ( ~t ), : ), 1 );
         cU = cU + sum ( Rcoeffs ( ~t ) .*  L ( Rterms ( ~t ), : ), 1 );
      end
      if any ( s )
         cL = cL + sum ( Fcoeffs ( s ) .* L ( Fterms ( s ), : ), 1 );
         cU = cU + sum ( Fcoeffs ( s ) .* U ( Fterms ( s ), : ), 1 );
      end
      if any ( ~s )
         cL = cL + sum ( Fcoeffs ( ~s ) .* U ( Fterms ( ~s ), : ), 1 );
         cU = cU + sum ( Fcoeffs ( ~s ) .* L ( Fterms ( ~s ), : ), 1 );
      end
      varci ( j, 1, : ) = max ( 0, cL );
      varci ( j, 2, : ) = max ( 0, cU );
   end   
