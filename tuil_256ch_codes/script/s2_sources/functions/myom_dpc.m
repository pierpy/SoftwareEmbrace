...
if nargin < 1
    ...
    figure
    hold on
    patch ( 'Vertices', squeeze ( postri ( x, :, : ) )', 'Faces', [ 1 2 3 ] )
    plot3 ( point (1), point (2), point (3), '.r', 'MarkerSize', 10 )
    
    nweight ( any ( weight < 0, 2 ) ) = 4;
    
    figure
    hold on
    patch ( 'Vertices', mesh.pos, 'Faces', mesh.tri, 'FaceVertexCData', nweight, 'FaceColor', 'flat', 'EdgeColor', 'none' )
